  ...
  return (
    <nav className="flex flex-wrap justify-between items-center z-10 relative">
      ...
      >
        <ul className="bg-gray-50 flex flex-col text-lg border border-gray-100 rounded-lg p-4 lg:flex-row lg:bg-transparent lg:border-none lg:space-x-8">
          {ROUTES.map((route, i) => {
            return (
              <li
                className={`lg:dark:text-white py-2 px-3 lg:hover:bg-transparent lg:hover:text-blue-500 cursor-pointer rounded ${i === 0 ? "bg-blue-500 text-white lg:bg-transparent lg:text-blue-500" : "hover:bg-gray-100"
      ...
      <div className="fixed left-4 bottom-4 lg:static lg:mr-8">
        <div onClick={ onClickShoppingButton} className="rounded-full bg-white shadow-md h-12 w-12 flex-center cursor-pointer btn-press-anim">
          <TbShoppingBag />
        </div>
      </div>
    </nav>
